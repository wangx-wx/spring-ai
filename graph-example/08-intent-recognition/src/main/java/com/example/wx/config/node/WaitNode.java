...
public class WaitNode implements AsyncNodeActionWithConfig, InterruptableAction {
    ...
    public Optional<InterruptionMetadata> interrupt(String nodeId, OverAllState state, RunnableConfig config) {
        ...
        var slotParams = state.value(inputKey, SlotFillingResult.class).orElse(SlotFillingResult.empty());

        if (StringUtils.hasText(slotParams.reply())) {
            state.input(Map.of(outputKey, slotParams.reply()));
        }
        if (isResume || "2".equals(slotParams.status())) {
            return Optional.empty();
        }
        return Optional.of(InterruptionMetadata.builder(nodeId, state)
                .addMetadata(outputKey, slotParams.reply())
                .build());
    }
}