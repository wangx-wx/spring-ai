...
```
04-nacos-mcp-gateway/
├── pom.xml                                    # Maven 配置
├── README.md                                  # 本文档
└── src/main/
    ├── java/com/example/wx/
    │   ├── NacosMcpGatewayApplication.java   # 启动类
    │   └── controller/
    │       └── GatewayController.java        # 测试控制器
    └── resources/
...
A: 检查以下几点：
1. Nacos 是否正常运行
2. MCP Server 是否已注册到 Nacos
3. `serviceNames` 配置是否正确

### Q: 调用工具失败？
A: 检查日志中的错误信息，可能原因：
1. MCP Server 未启动或不可达
2. 网络问题
3. 协议配置不匹配

### Q: 如何添加新的 MCP Server？
A: 在 `application.yaml` 的 `serviceNames` 中添加服务名，网关会在下次轮询时自动发现。
